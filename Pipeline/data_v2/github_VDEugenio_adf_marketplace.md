...
The ADF Marketplace serves as a centralized repository for Agent Definition Format files, which are specific to the Rawl ecosystem (https://rawl.ai/). ADF files are essentially portable AI agent containers packaged as SQLite databases that include the agent's configuration, memory, files, inbox/outbox, identity, and runtime state. This makes them completely self-contained and transferable between environments.

The project is currently deployed and accessible at https://adf-marketplace.vercel.app/, with the frontend hosted on Vercel and backend infrastructure on Render. However, the database is currently running on a free trial plan, which means data persistence is limited and will be upgraded soon for production stability.

The platform functions similarly to Hugging Face but is specifically designed for Rawl agents, allowing users to upload, browse, and download these specialized agent files. The marketplace includes features like GitHub OAuth authentication, file storage (supporting both local and AWS S3 backends), and a full-featured web interface for agent management and discovery.